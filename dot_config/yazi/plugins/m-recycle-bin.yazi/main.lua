...
function Notify.error(s, ...) Notify._send("error", s, ...) end
function Notify.warn(s, ...) Notify._send("warn", s, ...) end
function Notify.info(s, ...) Notify._send("info", s, ...) end
...
local function get_trash_directories()
  -- trashy doesn't expose a way to list all trash directories across mounts easily via CLI like trash-cli did.
  -- For now, we default to the standard home trash directory.
  -- Users with complex setups might need manual configuration if we wanted to support them,
  -- but adhering to XDG spec, valid trash dirs are predictable.
  local home_trash = os.getenv("HOME") .. "/.local/share/Trash/"
  local directories = { home_trash }
  
  debug("Using standard trash directory: %s", home_trash)
...
local function ensure_trash_directory(config)
  ...
  if dir_err then
    Notify.error(
      "Failed to discover trash directories: %s.",
      dir_err
    )
    return false
  end
  ...
  if #directories == 0 then
    Notify.error("No trash directories found. Please check trashy installation.")
    return false
  end
...
---Get all trash items by parsing .trashinfo files using ripgrep for performance
---@param config table
---@return table[] -- List of items with { filename, original_path, deletion_date, timestamp, size_bytes }
local function get_trash_items_from_info(config)
  local trash_dir = config.trash_dir
  if not trash_dir then return {} end
  
  if not trash_dir:match("/$") then trash_dir = trash_dir .. "/" end
  local info_dir = trash_dir .. "info"
  
  -- Use ripgrep to search for Path and DeletionDate in all .trashinfo files at once
  -- -o: only matching, -H: with filename, --no-heading: one line per match
  -- Pattern matches "Path=..." OR "DeletionDate=..."
  local cmd = Command("rg")
    :arg("-oH")
    :arg("--no-heading")
    :arg("(Path|DeletionDate)=.*")
    :arg(info_dir)
    :stdout(Command.PIPED)

  local output, err = cmd:output()
  if not output then
     debug("Failed to list info dir: %s", err)
     return {}
  end
  
  -- Parse output: /path/to/file.trashinfo:Path=/original/path
  local items_map = {}
  
  for line in output.stdout:gmatch("[^\r\n]+") do
    -- Parsing line: <filepath>:<Key>=<Value>
    -- filepath ends with .trashinfo
    local filepath, key, value = line:match("^(.*%.trashinfo):(.-)=(.*)$")
    
    if filepath and key and value then
      local filename = filepath:match("([^/]+)%.trashinfo$")
      
      if filename then
        if not items_map[filename] then items_map[filename] = { filename = filename } end
        
        if key == "Path" then
          items_map[filename].original_path = url_decode(value)
        elseif key == "DeletionDate" then
          -- Parse & Format Date
          -- Value: 2024-01-18T10:30:00
          local year, month, day, hour, min, sec = value:match("(%d+)-(%d+)-(%d+)T(%d+):(%d+):(%d+)")
          if year then
             items_map[filename].timestamp = os.time({
               year = tonumber(year), month = tonumber(month), day = tonumber(day),
               hour = tonumber(hour), min = tonumber(min), sec = tonumber(sec)
             })
             items_map[filename].deletion_date = string.format("%04d-%02d-%02d %02d:%02d:%02d", year, month, day, hour, min, sec)
          end
        end
      end
    end
  ...
  -- Convert map to list and filter incomplete items
  local items = {}
  for _, item in pairs(items_map) do
    if item.original_path and item.timestamp then
      table.insert(items, item)
    end
  end
  
  -- Sort by deletion date desc (newest first)
  table.sort(items, function(a, b) return a.timestamp > b.timestamp end)
  
  return items
end

---Get mapping of filenames to original paths parsing .trashinfo
---@param config table
---@return table<string, string>, string|nil -- filename_to_path_map, error
local function get_trash_file_mappings(config)
  local items = get_trash_items_from_info(config)
  local mappings = {}
  for _, item in ipairs(items) do
    mappings[item.filename] = item.original_path
  end
  debug("Created %d trash file mappings", #items)
  return mappings, nil
end
...
local function get_trash_files_with_sizes(config)
  ...
  -- We reuse existing get_files_with_sizes logic but need to pass just filenames
  local file_names = {}
  for _, item in ipairs(items) do
     table.insert(file_names, item.filename)
  end

  -- This gets sizes. 
  local sized_objects = get_files_with_sizes(file_names, trash_files_dir)
  
  debug("Retrieved %d trash files with sizes", #sized_objects)
  return sized_objects, nil
end
...
local function get_trash_files_older_than_days(config, days)
  ...
  for _, item in ipairs(items) do
    if item.timestamp < cutoff_time then
      table.insert(old_items, item)
      table.insert(old_filenames, item.filename)
    end
  end

  if #old_items == 0 then return {}, nil end

  -- Get file objects with sizes
  local trash_files_dir = get_trash_files_dir(config)
  local file_objects = get_files_with_sizes(old_filenames, trash_files_dir)

  -- Add deleted date information to file objects
  -- Note: file_objects should match order of old_filenames which matches old_items
  for i, file_obj in ipairs(file_objects) do
    if old_items[i] then
       file_obj.deleted_date = old_items[i].deletion_date
    end
  end
...
local is_current_dir_in_trash = function(config)
  ...
  if dir_err then
    Notify.error(
      "Failed to find trash directories: %s. Check trashy installation with 'trashy --version'",
      dir_err
    )
    return false
  end
...
local function get_trash_item_count(config)
  local items = get_trash_items_from_info(config)
  return #items, nil
end
...
local function get_trash_data(config)
  ...
  -- Get trash info
  local item_count, count_err = get_trash_item_count(config)
  if count_err then
...
local function cmd_empty_trash(config)
  ...
  -- Execute trashy empty command
  local err, _ = run_command("trashy", { "empty", "--all", "--force" })
  if err then
    Notify.error("Failed to empty trash: %s. Try 'trashy empty --all' manually to debug", err)
    return
  end
...
local function cmd_empty_trash_by_days(config)
  ...
  -- Execute trashy empty for each file
  -- Since trashy doesn't support "days" directly, we iterate and delete by path
  -- We already have the list of files to delete in `file_objects`.
  -- However, we need their original paths to pass to `trashy empty` effectively or we can just empty by absolute path of the trash file if trashy supports it? 
  -- Trashy empty takes original path. 
  
  -- Re-get items to map filename to original path
  local all_items = get_trash_items_from_info(config)
  local name_to_path = {}
  for _, item in ipairs(all_items) do name_to_path[item.filename] = item.original_path end

  local success_count = 0
  local fail_count = 0

  for _, obj in ipairs(file_objects) do
     local original_path = name_to_path[obj.name]
     if original_path then
       -- trashy empty <path> matches against original path regex.
       -- Escape special chars in path to treat it as literal string if possible, or hopefully it just works.
       -- Using `trashy empty "path"` might match partials.
       -- Safest might be to match exact line.
       local err, _ = run_command("trashy", { "empty", original_path, "--force" })
       if err then 
  ...
  if fail_count > 0 then
    Notify.warn("Failed to delete %d items", fail_count)
    return
  end
  local err = nil -- Clear error if we handled it manually above
  if err then
...
local function cmd_delete_selection(config)
  ...
  local function delete_operation(path)
    ...
    -- Use trashy empty with the component name
    -- FIXME: Trashy empty uses original path regex.
    -- If we have the original path available we should use it. 
    -- But here we only have the filename in `path` (the trash file path).
    -- We need to resolve it to original path.
    local items = get_trash_items_from_info(config)
    ...
    if not original_path then
       return "Could not find original path for " .. filename
    end

    local delete_err, _ = run_command("trashy", { "empty", original_path, "--force" })
    if delete_err then
...
local function cmd_restore_selection(config)
  ...
  -- Get trash file mappings parsing .trashinfo
  local trash_mappings, mapping_err = get_trash_file_mappings(config)
  if mapping_err then
    Notify.error("Failed to get trash mappings: %s. Try 'trashy list' manually to verify trash contents", mapping_err)
    return
  end
  ...
  local function restore_operation(item)
    ...
    -- Use trashy restore with the original file path as argument
    local restore_err, _ = run_command("trashy", { "restore", item.original_path, "--force" })
    if restore_err then
...
local function check_dependencies()
  -- Check for trashy
  local trashy_err, _ = run_command("trashy", { "--version" }, nil, true)
  if trashy_err then
    local path = os.getenv("PATH") or "(unset)"
    Notify.error("trashy not found. Is it installed and in PATH? PATH=" .. path)
    return false
  end
  
  -- Check for ripgrep
  local rg_err, _ = run_command("rg", { "--version" }, nil, true)
  if rg_err then
    Notify.error("ripgrep (rg) not found. Is it installed and in PATH?")
    return false
  end
...
local default_config = {
  trash_dir = nil, -- Will be auto-discovered
  os = ya.target_os(),
}
